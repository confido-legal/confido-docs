...
This is a reference guide for the Hosted Fields SDK. The SDK is a javascript file which should be loaded via a `script` tag on any page you want to load our hosted fields. Once the script loads the `window` object will have a `window.confidoHostedFields` object with the methods documented here. The `confidoHostedFields` object is a singleton. This means you can only having one instance of hosted fields per `window`.
