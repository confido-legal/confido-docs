...
For a more detailed explanation of the `submitFields()` method, see the [Reference Docs](/reference/hosted-fields-js/submit-fields).

```javascript
window.gravityLegal.submitFields(async (err) => {
    if (err) {
