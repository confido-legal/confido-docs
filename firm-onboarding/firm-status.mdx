...
When a firm is onboarding, the `firm.status` field will go through the following values. A `firm.updated` [webhook](/docs/webhooks/overview) will be sent out every time the status changes.
